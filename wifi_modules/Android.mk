...
export REALTOP=$(realpath $(TOP))
wifi_modules: android_kernel
	mkdir -p $(PRODUCT_OUT)/obj/wifi-build &&\
	cd $(PRODUCT_OUT)/obj/wifi-build &&\
	export TOOLCHAIN_URL=https://releases.linaro.org/15.02/components/toolchain/binaries/aarch64-linux-gnu/gcc-linaro-4.9-2015.02-3-x86_64_aarch64-linux-gnu.tar.xz &&\
	if [ ! -d ${TOOLCHAIN_DIR} ] ; then wget ${TOOLCHAIN_URL}; tar -xJf ${TOOLCHAIN_TARBALL}; fi &&\
	export PATH=${PATH}:${PWD}/${TOOLCHAIN_DIR}/bin/ &&\
	cp -rf $(REALTOP)/kernel/linaro/hisilicon/ kernel/ &&\
	cp -rf $(REALTOP)/$(PRODUCT_OUT)/obj/kernel/. kernel/ &&\
	cd kernel/ &&\
	...
	sed -e "s|^export TOOLCHAIN_PATH=.*|export TOOLCHAIN_PATH=$(REALTOP)/$(PRODUCT_OUT)/obj/wifi-build/${TOOLCHAIN_DIR}/bin|" -e "s|^export KERNEL_PATH=.*|export KERNEL_PATH=$(REALTOP)/$(PRODUCT_OUT)/obj/wifi-build/kernel|" -e "s|^export CROSS_COMPILE=.*|export CROSS_COMPILE=aarch64-linux-gnu-|" -e "s|^export ARCH=.*|export ARCH=arm64|" -i build-utilites/setup-env &&\
	...
	mkdir -p $(REALTOP)/$(PRODUCT_OUT)/system/modules/wifi
